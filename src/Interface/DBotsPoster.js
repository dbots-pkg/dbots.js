 ...
 */
class DBotsPoster extends EventEmitter {
    constructor(options){
    	super();
    	if (!options || options.constructor.name !== 'Object') throw new Error("An Object is required to construct a poster.");
        this.client = options.client;
        if(client && !options.clientLibrary && !options.serverCount) throw new Error("clientID or serverCount must be defined when a client is defined.");
        if(!client && !options.clientID) throw new Error("clientID must be defined when client is non-existant.");
    	if(client && !options.clientID) Object.assign(options, Constants.AutoValueFunctions(options.clientLibrary));
    ...
    getServerCount(){
        ...
        return Constants.ServerCountFunctions[this.options.clientLibrary](this.client);
    }
	...
	/**
	  ...
	  * @param {String} service The service to post to
	  * @returns {Promise, Array<Promise>} The result(s) of the post
	  */
    postManual(serverCount, service = 'all'){
    	if(!this.options.apiKeys) throw new Error('NO_API_KEYS');
        if(!service || service == 'all') return Promise.all(Object.keys(this.options.apiKeys).map(this.post));
        ...
        return Constants.PostFormat[service](this.options.apiKeys[service], this.options.clientID, serverCount, this.options.shard)
    }
}

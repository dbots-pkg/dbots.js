...
var DiscordBotList = /** @class */ (function (_super) {
    ...
    DiscordBotList.post = function (options) {
        ...
        var data = { guilds: Util_1.default.resolveCount(serverCount) };
        if (shard)
            data.shard_id = shard.id;
        if (userCount)
            data.users = Util_1.default.resolveCount(userCount);
        if (voiceConnections)
            data.voice_connections = Util_1.default.resolveCount(voiceConnections);
